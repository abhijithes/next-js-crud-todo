            ...
            <ul className="p-5">
                ...
                {topics && topics?.map((topic) => (
                    <li className="flex justify-between mb-3 border-2 border-gray-300 rounded-lg shadow-lg " key={topic._id}>
                        <div className="my-2"><strong className="mt-2 ml-2">{topic.title}</strong><p className="ml-2">{topic.description}</p></div>
                        <div>
                            <button className="font-bold mx-2 bg-red-500 p-2 m-2 rounded-xl" onClick={()=>{
                                try{
                            ...
                            <button className="mx-2 font-bold bg-blue-400 p-2 m-2 rounded-xl" onClick={()=>{router.push(`/todo/edittopic/${topic._id}`)}}>edit</button>
                        </div>
                    </li>
                ))}
            </ul>
